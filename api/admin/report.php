 ...
 * Params:
 *   view: 'day' | 'month' | 'year'
 *   year: 2026-2036 (for month/day view)
...
// Validate (dynamic: current year to +11)
...
if ($view === 'year') {
    // ============================================
    // YEARLY REPORT: Show data per year (dynamic range)
    // ============================================
    ...
    // Walk-in sales by year (exclude voided)
    $stmtW = $db->prepare("
        SELECT YEAR(created_at) as period, COALESCE(SUM(amount), 0) as revenue, COUNT(*) as count
        FROM walkin_sales
        WHERE YEAR(created_at) BETWEEN ? AND ? AND status = 'active'
        GROUP BY YEAR(created_at)
    ");
    ...
    // Build response for years (dynamic range)
    $rows = [];
    ...
    // ============================================
    // MONTHLY REPORT: Show 12 months for given year
    // ============================================
    ...
    // Walk-in sales by month (exclude voided)
    $stmtW = $db->prepare("
    ...
    // ============================================
    // DAILY REPORT: Show all days for given month/year
    // ============================================
    ...
    // Walk-in sales by day (exclude voided)
    $stmtW = $db->prepare("
        SELECT DAY(created_at) as period, COALESCE(SUM(amount), 0) as revenue, COUNT(*) as count
        FROM walkin_sales
        WHERE YEAR(created_at) = ? AND MONTH(created_at) = ? AND status = 'active'
